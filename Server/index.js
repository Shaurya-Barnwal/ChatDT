...
io.on('connection', socket => {
  ...
  socket.on('join-room', async ({ roomId, userId, username }) => {
    try {
      ...
      socket.emit('recent-messages', res.rows);
    } catch (err) {
  ...
  socket.on('send-message', async ({ roomId, userId, username, ciphertext, iv, messageId }) => {
    try {
      // ensure user exists
      await pool.query(
      ...
      const insert = await pool.query(
        `INSERT INTO messages (message_id, room_id, sender_id, ciphertext, iv)
         VALUES ($1,$2,$3,$4,$5) RETURNING *;`,
        [messageId, roomId, userId, ciphertext, iv]
      );

      // fetch name
      const nameRes = await pool.query('SELECT name FROM users WHERE id=$1', [userId]);
      const senderName = nameRes.rows[0]?.name || username || 'Anon';
